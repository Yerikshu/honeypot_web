...
opencanary_web_folder="/usr/local/src/honeypot"
...
if [ "$opencanary_web_mysql_password" = "$huanchengzijidemima" ]; then    
    mysql -u root -e "
     ...
     flush privileges;
     use honeypot;
     source /usr/local/src/honeypot/honeypot.sql;
     use mysql;
...
#get_mail_addressee=`sed -n '2p' /usr/local/src/honeypot/util/conf/email.ini`
...
echo "已成功安装opencanary_web,文件路径:/usr/local/src/honeypot"
...
echo "以及修改/usr/local/src/honeypot/dbs/initdb.py,DB_USER/DB_PWD字段"
echo "已经配置成功蜜罐告警邮件,具体配置浏览/usr/local/src/honeypot/application.py"
echo "收件人邮件配置(以及告警开关):/usr/local/src/honeypot/util/conf/email.ini"
echo "更多信息请参考https://github.com/p1r06u3/honeypot"
;;
n*)
echo "已同步cn.pool.ntp.org时间,已关闭SELINUX和防火墙"
...
echo "蜜罐告警具体配置(发件人)浏览/usr/local/src/honeypot/application.py"
echo "收件人邮件配置(以及告警开关):/usr/local/src/honeypot/util/conf/email.ini"
echo "更多信息请参考https://github.com/p1r06u3/honeypot"
;;
*)
